...
def RemeetTimes(mAdj):
    ...
    try:
        result, info = bicgstab(L, s, tol=1e-3)
        if info == 0:
...
df = pd.read_csv("network input/Food ingredients and flavors_w.csv")
G = nx.from_pandas_edgelist(df, 'source', 'target', 'weight')
N = G.number_of_nodes()
node_map = dict(zip(G.nodes, range(N)))
adjacency_matrix_sparse = nx.adjacency_matrix(G, nodelist=node_map.keys()).todense()
adjacency_matrix_triu = np.triu(adjacency_matrix_sparse, k=1)
adjacency_matrix_triu += adjacency_matrix_triu.T - np.diag(adjacency_matrix_triu.diagonal())

M_adj = csr_matrix(adjacency_matrix_triu)
print('m_adj is', M_adj)
print(M_adj.shape)
...
tft = [0, 0.99, 0.99, 0.01]
wsls = [0, 0.99, 0.01, 0.99]
grim = [0, 0.99, 0.01, 0.01]
sco = [1, 0.99, 0.99, 0.01]
sj = [1, 0.99, 0.01, 0.99]
sh = [1, 0.99, 0.01, 0.01]
s1_list = [tft, wsls, grim, sco, sj, sh]
...
for l in range(6):
    ...
    print('bc ratio is', b_to_c_ratio)
...
index_2 = ['tft', 'wsls', 'grim', 'sco', 'sj', 'sh']
results_df = pd.DataFrame(data=results, index=index_2, columns=['bc ratio'])
results_df2 = results_df.dropna(axis=0, how='any')
results_df2.to_excel('fig5_Food_w.xlsx') 

...
# Simulation
    ...
    while n_cooperators > 0 and n_cooperators < N: 
        ...
        counter += 1
        results_cooperation.append(n_cooperators) 
    
    return results_cooperation
...
def run_simulation(n_runs):
    ...
    for i in range(n_runs):
        ...
        result = run_until_fixation(graph, N, first_cooperator_index, iteration_type)
        if result[-1] == N:
            all_results.append(result)

    # 将结果转换为等长的numpy数组
    max_length = max(len(res) for res in all_results)
    padded_results = np.array([res + [N]*(max_length - len(res)) for res in all_results])
    #print('padd', padded_results)

    # 计算每个时间点的平均合作者数量和标准差
    means = np.mean(padded_results, axis=0)
    std_devs = np.std(padded_results, axis=0)
...
d_s_q = np.zeros((4, 1))
alpha_list = [0.1, 0.2, 0.4, 0.6, 0.8]
delta_2 = 0.98
...
for alpha in alpha_list: 
    mean_results, std_dev_results = run_simulation(n_runs)
    print('alpha', alpha)
    #print('Average number of cooperators at each step:', mean_results)
    #print('Standard deviation of cooperators at each step:', std_dev_results)
    results_total.append(mean_results)
    results_total.append(std_dev_results)

inde = ['mean 0.1', 'std 0.1', 'mean 0.2', 'std 0.2', 'mean 0.4', 'std 0.4', 'mean 0.6', 'std 0.6', 'mean 0.8', 'std 0.8']
df = pd.DataFrame(results_total, index=inde).T
df.to_excel('C_T_RR.xlsx')

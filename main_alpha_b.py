...
s1 = [la, 1., 1., 1.]
s2 = [la, 0., 0., 0.]
benefit_list = np.linspace(1.1, 4, 40)

#q_i = np.array([[0.], [0.], [0.], [0.]])
#d_s_q = np.zeros((4, 1))
alpha_list = np.linspace(0.01, 0.99, 20)
delta_2 = 0.98
...
results = np.array(results).reshape(-1, len(benefit_list))

results_df = pd.DataFrame(results)

results_df.to_excel('phi_100(4)_ba_map.xlsx')

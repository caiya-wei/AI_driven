...
def good_probability(k_s1, s1, s2, d, eps):
    ...
    m11 = 1 - w_ij - la1 * (2. / k_aver - w_ij) + w_ij * r1 * (1. - 2. * eps + la1 * (1. - 2. * eps) * (k_s1 - 2.))
    ...
    m44 = 1. - w_ij - la2 * (2. / k_aver - w_ij) + w_ij * r2 * (1. - 2. * eps) + w_ij * r2 * la2 * (1. - 2. * eps) * (k_aver - k_s1 - 2.)
...
results = []
list_eps = np.linspace(0.001, 0.1, num=50)
list_delta = np.linspace(0.01, 0.99, num=100)
...
for eps in list_eps:
    for delta in list_delta:
        ...
        k = 58
        p = 0.1
        G = nx.gnp_random_graph(500, 0.2)
        degree = np.array([val for (node, val) in G.degree()])
        k_mean = np.mean(degree)
        i = np.random.randint(0, 499)
        k_aver = degree[i]
        # noisy_degree = degree + np.random.normal(0, 10, size=degree.shape)
        #k_aver = np.mean(noisy_degree)
        #print(k_aver)
        #k_aver = sum(dict(G.degree()).values()) / float(len(G))
    ...
    # s3 = [la, 0.99, 0.99, q1]
        ...
        relative_ratio = (b_to_c_ratio - k_mean) / k_mean
